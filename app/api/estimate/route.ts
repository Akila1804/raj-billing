...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: insertError } = await supabase.from("estimate").insert([
      {
        estimationNo: form.estimationNo,
        date: form.date,
    ...
    return NextResponse.json(
      { message: "Estimation saved successfully", success: true },
      { status: 201 },
    );
  } catch (error) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (id) {
      ...
      ({ data, error } = await supabase.from("estimate").select("*"));
    }
    if (error) {
